...
return{
	get_all_supp: function(req, res){
		console.log("getting all supp from database: ");
	...
	add_supp: function(req, res){
		console.log("submit recording of a supp: ");

		var array = req.params.supp.split("-");
		console.log(array);

		var key = array[0]
		var emailaddress = array[1]
		var name = array[2]
		var role = array[3]
		var holder = array[4]
		var balance = array[5]
	...
	get_supp: function(req, res){
		...
		Fabric_Client.newDefaultKeyValueStore({ path: store_path
		    ...
		    if (query_responses && query_responses.length == 1) {
		        if (query_responses[0] instanceof Error) {
		            console.error("error from query = ", query_responses[0]);
		            res.send("Could not locate supp")
	...
	// 	console.log("changing holder of catch: ");
